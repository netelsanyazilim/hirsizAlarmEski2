...
@observer
class AudioFeedback extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            languages: [
                ...
                { label: langObj.onlyWithRemote, value: 2 }
            ],
            value1Index: 0,
            flagSettings: null
        };
        ...
        NetInfo.fetch().then(state => {
            if (state.isConnected) {
                ApiCall.getSettings().then((x) => {
                    if (x) {
                        this.setState({
                            value1Index: x.sesliGeriBildirim,
                            flagSettings: x
                        });
                            ...
                            title={langObj.submit} onPress={() => {
                                NetInfo.fetch().then(state => {
                                    if (state.isConnected) {
                                        ...
                                        flagSettings.sesliGeriBildirim = this.state.value1Index;
