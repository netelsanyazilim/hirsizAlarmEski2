...
@observer
class PhoneCallLineSelection extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            languages: [
                { label: langObj.pttLine, value: 0 },
                { label: langObj.gsmLine, value: 1 }
            ],
            value1Index: 0,
            flagSettings: null
        };
        ...
        NetInfo.fetch().then(state => {
            if (state.isConnected) {
                ApiCall.getSettings().then((x) => {
                    if (x) {
                        this.setState({
                            value1Index: (x.aramaHatSecimi - 1),
                            flagSettings: x
                        });
                ...
                {!this.visibleSpinner && <View style={{ justifyContent: 'space-between', flex: 1, backgroundColor: 'white' }} >
                    <View>
                        <Text style={{ fontWeight: "bold", alignSelf: "center", margin: 10, color: 'gray', fontSize: 20 }}>
                            {langObj.phoneCallLineSelection}
                        </Text>
                            ...
                            title={langObj.submit} onPress={() => {
                                NetInfo.fetch().then(state => {
                                    if (state.isConnected) {
                                        ...
                                        flagSettings.aramaHatSecimi = (this.state.value1Index + 1);
...
export default PhoneCallLineSelection;