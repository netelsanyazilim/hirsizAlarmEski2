...
@observer
class SirenSoundingDuration extends Component {
        ...
        NetInfo.fetch().then(state => {
            if (state.isConnected) {
                ApiCall.getSettings().then((x) => {
                    if (x) {
                        this.setState({
                            sirenSoundingDuration: x.sirenSuresi,
                            flagSettings: x
                        });
                        ...
                        <View style={{
                            ...
                            <View style={{ flexDirection: 'row', marginLeft: 15, }}>
                                <Icon size={18} name="plus" color="gray" type="font-awesome"
                                    ...
                                    onPress={() => {
                                        if ((Number.parseInt(this.state.sirenSoundingDuration) + 1) < 256) {
                                            this.setState({ sirenSoundingDuration: (Number.parseInt(this.state.sirenSoundingDuration) + 1).toString() });
                                        } else {
                                            this.setState({ sirenSoundingDuration: "255" });
                                ...
                                <TextInput style={styles.textInput} textAlign={'center'} autoCapitalize={"none"} autoCorrect={false}
                                    value={this.state.sirenSoundingDuration.toString()} onChangeText={(text) => {
                                        const parsedQty = Number.parseInt(text);
                                        if (!Number.isNaN(parsedQty) && text != "" && text != null) {
                                            if ((parsedQty < 256) && (parsedQty > 0)) {
                                                this.setState({ sirenSoundingDuration: parsedQty.toString() });
                                            } else {
                                                this.setState({ sirenSoundingDuration: "1" });
                                                Toast.show(langObj.minAndMaxValue, Toast.LONG);
                                            }
                                        } else {
                                            this.setState({ sirenSoundingDuration: "" });
                                        }
                                    }}
                                ...
                                <Icon size={18} name="minus" type="font-awesome" color="gray"
                                    containerStyle={Platform.OS === 'ios' ? { marginTop: 10 } : { marginTop: 20 }}
                                    onPress={() => {
                                        if ((Number.parseInt(this.state.sirenSoundingDuration) - 1) > 0) {
                                            this.setState({ sirenSoundingDuration: (Number.parseInt(this.state.sirenSoundingDuration) - 1).toString() });
                                        } else {
                                            this.setState({ sirenSoundingDuration: "1" });
                                ...
                                />
                                <Text style={{ alignSelf: 'center', marginLeft: 10 }} >{langObj.system3Unit}</Text>
                            </View>
                            <View>
                                    ...
                                    onPress={() => {
                                        NetInfo.fetch().then(state => {
                                            if (state.isConnected) {
                                                ...
                                                flagSettings.sirenSuresi = this.state.sirenSoundingDuration;
...
export default SirenSoundingDuration;