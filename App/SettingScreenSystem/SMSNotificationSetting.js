...
@observer
class SMSNotificationSetting extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            languages: [
                { label: langObj.passive, value: 0 },
                { label: langObj.active, value: 1 }
            ],
            value1Index: 0,
            flagSettings: null
        };
                ...
                {!this.visibleSpinner && <View style={{ justifyContent: 'space-between', flex: 1, backgroundColor: 'white' }} >
                    <View>
                        <Text style={{ fontWeight: "bold", alignSelf: "center", margin: 10, color: 'gray', fontSize: 20 }}>
                            {langObj.smsNotificationSetting}
                        </Text>
                            ...
                            title={langObj.submit} onPress={() => {
                                NetInfo.fetch().then(state => {
                                    if (state.isConnected) {
                                        ...
                                        flagSettings.smsBildirim = this.state.value1Index;
...
export default SMSNotificationSetting;