...
@observer
class NumberCallRepetitions extends Component {
        ...
        NetInfo.fetch().then(state => {
            if (state.isConnected) {
                ApiCall.getSettings().then((x) => {
                    if (x) {
                        this.setState({
                            numberCallRepetitions: x.aramaTekrarSayisi,
                            flagSettings: x
                        });
                        ...
                        <View style={{
                            ...
                            <View style={{ flexDirection: 'row', marginLeft: 15, }}>
                                <Icon size={18} name="plus" color="gray" type="font-awesome"
                                    ...
                                    onPress={() => {
                                        if ((Number.parseInt(this.state.numberCallRepetitions) + 1) < 11) {
                                            this.setState({ numberCallRepetitions: (Number.parseInt(this.state.numberCallRepetitions) + 1).toString() });
                                        } else {
                                            this.setState({ numberCallRepetitions: "10" });
                                            Toast.show(langObj.maxCallLimit, Toast.LONG);
                                        }
                                    }}
                                />
                                <TextInput style={styles.textInput} textAlign={'center'} autoCapitalize={"none"} autoCorrect={false}
                                    value={this.state.numberCallRepetitions.toString()} onChangeText={(text) => {
                                        const parsedQty = Number.parseInt(text);
                                        if (!Number.isNaN(parsedQty) && text != "" && text != null) {
                                            if ((parsedQty < 11) && (parsedQty > 0)) {
                                                this.setState({ numberCallRepetitions: parsedQty.toString() });
                                            } else {
                                                this.setState({ numberCallRepetitions: "1" });
                                                Toast.show(langObj.minAndMaxValue2, Toast.LONG);
                                            }
                                        } else {
                                            this.setState({ numberCallRepetitions: "" });
                                        }
                                    }}
                                ...
                                <Icon size={18} name="minus" type="font-awesome" color="gray"
                                    containerStyle={Platform.OS === 'ios' ? { marginTop: 10 } : { marginTop: 20 }}
                                    onPress={() => {
                                        if ((Number.parseInt(this.state.numberCallRepetitions) - 1) > 0) {
                                            this.setState({ numberCallRepetitions: (Number.parseInt(this.state.numberCallRepetitions) - 1).toString() });
                                        } else {
                                            this.setState({ numberCallRepetitions: "1" });
                                    ...
                                    onPress={() => {
                                        NetInfo.fetch().then(state => {
                                            if (state.isConnected) {
                                                ...
                                                flagSettings.aramaTekrarSayisi = this.state.numberCallRepetitions;
...
export default NumberCallRepetitions;