...
@observer
class AlarmRepeatCount extends Component {
        ...
        NetInfo.fetch().then(state => {
            if (state.isConnected) {
                ApiCall.getSettings().then((x) => {
                    if (x) {
                        this.setState({
                            alarmRepeatTimes: x.alarmTekrarSayisi,
                            flagSettings: x
                        });
                        ...
                        <View style={{
                            ...
                            <View style={{ flexDirection: 'row', marginLeft: 15, }}>
                                <Icon size={18} name="plus" color="gray" type="font-awesome"
                                    ...
                                    onPress={() => {
                                        if ((Number.parseInt(this.state.alarmRepeatTimes) + 1) < 256) {
                                            this.setState({ alarmRepeatTimes: (Number.parseInt(this.state.alarmRepeatTimes) + 1).toString() });
                                        } else {
                                            this.setState({ alarmRepeatTimes: "255" });
                                ...
                                <TextInput style={styles.textInput} textAlign={'center'} autoCapitalize={"none"} autoCorrect={false}
                                    value={this.state.alarmRepeatTimes.toString()} onChangeText={(text) => {
                                        const parsedQty = Number.parseInt(text);
                                        if (!Number.isNaN(parsedQty) && text != "" && text != null) {
                                            if ((parsedQty < 256) && (parsedQty > 0)) {
                                                this.setState({ alarmRepeatTimes: parsedQty.toString() });
                                            } else {
                                                this.setState({ alarmRepeatTimes: "1" });
                                                Toast.show(langObj.minAndMaxValue, Toast.LONG);
                                            }
                                        } else {
                                            this.setState({ alarmRepeatTimes: "" });
                                        }
                                    }}
                                ...
                                <Icon size={18} name="minus" type="font-awesome" color="gray"
                                    containerStyle={Platform.OS === 'ios' ? { marginTop: 10 } : { marginTop: 20 }}
                                    onPress={() => {
                                        if ((Number.parseInt(this.state.alarmRepeatTimes) - 1) > 0) {
                                            this.setState({ alarmRepeatTimes: (Number.parseInt(this.state.alarmRepeatTimes) - 1).toString() });
                                        } else {
                                            this.setState({ alarmRepeatTimes: "1" });
                                    ...
                                    onPress={() => {
                                        NetInfo.fetch().then(state => {
                                            if (state.isConnected) {
                                                ...
                                                flagSettings.alarmTekrarSayisi = this.state.alarmRepeatTimes;
...
export default AlarmRepeatCount;