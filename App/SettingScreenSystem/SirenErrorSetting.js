...
@observer
class SirenErrorSetting extends Component {
        ...
        NetInfo.fetch().then(state => {
            if (state.isConnected) {
                ApiCall.getSettings().then((x) => {
                    if (x) {
                        this.setState({
                            value1Index: x.sirenHataKontrol,
                            flagSettings: x
                        });
                            ...
                            title={langObj.submit} onPress={() => {
                                NetInfo.fetch().then(state => {
                                    if (state.isConnected) {
                                        ...
                                        flagSettings.sirenHataKontrol = this.state.value1Index;
...
export default SirenErrorSetting;