...
@observer
class SectionSelectionRemote extends Component {
    ...
    getRemotePart() {
        NetInfo.fetch().then(state => {
            if (state.isConnected) {
                ApiCall.getRemotePart(global.deviceId).then((x) => {
                    if (x) {
                        if (x != undefined && x != null) {
                            ...
                            for (let index = 0; index < x.length; index++) {
                                ...
                                flagRemotes.push(base2);
                            }
                            this.remotes = flagRemotes;
                        } else {
                            this.remotes = [];
                        }
                    } else {
        ...
        return (
            <View style={{ marginTop: 10, elevation: 5 }}>
                <View style={{ flexDirection: "row", alignSelf: "center", width: '100%', justifyContent: "space-between" }}>
                    <View>
                        <Text style={{ fontWeight: "bold", margin: 10, marginLeft: 20, color: 'black', fontSize: 18 }}>{langObj.remoteControl + " " + index}</Text>
                    </View>
                    <View style={{ flexDirection: "row", alignSelf: "center", marginTop: 5, marginBottom: 5, justifyContent: "space-between" }}>
                        ...
                        <BouncyCheckbox
                            fillColor="green"
                            unfillColor="#FFFFFF"
                            isChecked={(this.remotes[index - 1].charAt(0) == "1") ? true : false}
                            onPress={() => {
                                if ((this.remotes[index - 1].charAt(0) == "1"))
                                    this.remotes[index - 1] = this.replaceAt(this.remotes[index - 1], 0, "0");
                                else
                                    this.remotes[index - 1] = this.replaceAt(this.remotes[index - 1], 0, "1");
                            }}
                        />
                        ...
                        <BouncyCheckbox
                            fillColor="green"
                            unfillColor="#FFFFFF"
                            isChecked={(this.remotes[index - 1].charAt(1) == "1") ? true : false}
                            onPress={() => {
                                if ((this.remotes[index - 1].charAt(1) == "1"))
                                    this.remotes[index - 1] = this.replaceAt(this.remotes[index - 1], 1, "0");
                                else
                                    this.remotes[index - 1] = this.replaceAt(this.remotes[index - 1], 1, "1");
                            }}
                        />
                        ...
                        <BouncyCheckbox
                            fillColor="green"
                            unfillColor="#FFFFFF"
                            isChecked={(this.remotes[index - 1].charAt(2) == "1") ? true : false}
                            onPress={() => {
                                if ((this.remotes[index - 1].charAt(2) == "1"))
                                    this.remotes[index - 1] = this.replaceAt(this.remotes[index - 1], 2, "0");
                                else
                                    this.remotes[index - 1] = this.replaceAt(this.remotes[index - 1], 2, "1");
                            }}
                        />
                        ...
                        <BouncyCheckbox
                            fillColor="green"
                            unfillColor="#FFFFFF"
                            isChecked={(this.remotes[index - 1].charAt(3) == "1") ? true : false}
                            onPress={() => {
                                if ((this.remotes[index - 1].charAt(3) == "1"))
                                    this.remotes[index - 1] = this.replaceAt(this.remotes[index - 1], 3, "0");
                                else
                                    this.remotes[index - 1] = this.replaceAt(this.remotes[index - 1], 3, "1");
                            }}
                        />
                    </View>
                </View>
                        ...
                        <Button
                            ...
                            title={langObj.submit} onPress={() => {
                                NetInfo.fetch().then(state => {
                                    if (state.isConnected) {
                                        let flagData = [];
                                        for (let index = 0; index < this.remotes.length; index++) {
                                            flagData.push(parseInt(this.remotes[index].split("").reverse().join(""), 2));
                                        }
                                        ApiCall.setRemotePart(flagData).then((x) => {
                                            if (x) {
                                                this.getRemotePart();
                                            }
                                        });
...
export default SectionSelectionRemote;