...
class SectionMasterPassword extends Component {
                ...
                <View style={{ flex: 1, marginTop: 10, alignItems: 'center' }} >
                    <View style={{ width: '100%', alignItems: 'center' }}>
                        <Text style={{ fontSize: 20, textAlign: 'center', backgroundColor: 'transparent', fontWeight: 'bold', color: 'gray' }}>
                            {langObj.changeSectionMasterPasswordPage}
                        </Text>
                        <TextInput style={styles.textInput2}
                            textAlign={'center'}
                            autoCapitalize={"none"}
                            autoCorrect={false}
                            value={this.state.sectionNumber.toString()}
                            onChangeText={(text) => {
                                const parsedQty = Number.parseInt(text);
                                if (!Number.isNaN(parsedQty) && text != "" && text != null) {
                                    if ((parsedQty < 5) && (parsedQty > 0)) {
                                        this.setState({ sectionNumber: parsedQty.toString() });
                                    } else {
                                        this.setState({ sectionNumber: "" });
                                        Toast.show(langObj.maxSectionNumber, Toast.LONG);
                                    }
                                } else {
                                    this.setState({ sectionNumber: "" });
                                }
                            ...
                            maxLength={1}
                            placeholder={langObj.sectionNumber}
                            ...
                            onPress={() => {
                                NetInfo.fetch().then(state => {
                                    if (!(this.passwords.includes(this.state.newPassword)) || (this.state.newPassword == "0000")) {
                                        if (state.isConnected) {
                                            ApiCall.setPassword(1, this.state.sectionNumber, this.state.newPassword).then((x) => {
                                                if (x) {
                                                    this.getAllPasswords();
                                                    this.setState({ sectionNumber: "", newPassword: "" });
                                                }
                                            });
                                        }
...
export default SectionMasterPassword;